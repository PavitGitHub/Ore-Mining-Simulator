...
public class MapGrid
{
  ...
  private ElementType[][] mapElements; // = new OreSim.ElementType[nbHorzCells][nbVertCells];
  ...
  public MapGrid(int model) {
    ...
    mapElements = new ElementType[nbHorzCells][nbVertCells];
    ...
    for (int k = 0; k < nbVertCellsModel[model]; k++)
    {
      for (int i = 0; i < nbHorzCellsModel[model]; i++)
      {
        ...
        {
          case ' ':
            mapElements[i][k] = ElementType.OUTSIDE;  // Empty outside
            break;
          case '.':
            mapElements[i][k] = ElementType.EMPTY;  // Empty inside
            break;
          case 'x':
            mapElements[i][k] = ElementType.BORDER;  // Border
            break;
          case '*':
            mapElements[i][k] = ElementType.ORE;  // Stones
            nbStones++;
            break;
          case 'o':
            mapElements[i][k] = ElementType.TARGET;  // Target positions
            break;
          case 'P':
            mapElements[i][k] = ElementType.PUSHER;
            break;
          case 'B':
            mapElements[i][k] = ElementType.BULLDOZER;
            break;
          case 'E':
            mapElements[i][k] = ElementType.EXCAVATOR;
            break;
          case 'R':
            mapElements[i][k] = ElementType.ROCK; // Rocks
            break;
          case 'D':
            mapElements[i][k] = ElementType.CLAY; // Clay
            break;
        }
      }
    }
  ...
  public ElementType getCell(Location location)
  {
